...
class Agent:
    def __init__(self, n_states, n_actions, n_goals, action_bounds, capacity, env,
        ...
        self.demonstrator = Actor(self.n_states, n_actions=self.n_actions, n_goals=self.n_goals).to(self.device)  # --
        ...
        # self.sync_networks(self.actor_h)  # -- no need to sync actor_h as it is single threaded
        ...
        self.demonstrator_target = Actor(self.n_states, n_actions=self.n_actions, n_goals=self.n_goals).to(
            self.device)  # --
        ...
        self.demonstrator_lr = demonstrator_lr
        self.critic_lr = critic_lr
        self.actor_optim = Adam(self.actor.parameters(), self.actor_lr)
        self.demonstrator_optim = Adam(self.demonstrator.parameters(), self.demonstrator_lr)  # --
    ...
    # --------------------------------------------------------------------
    def choose_action_demonstrator(self, state, goal, train_mode=True):
        state = self.state_normalizer.normalize(state)
        goal = self.goal_normalizer.normalize(goal)
        state = np.expand_dims(state, axis=0)
        goal = np.expand_dims(goal, axis=0)
        ...
        with torch.no_grad():
            x = np.concatenate([state, goal], axis=1)
            x = from_numpy(x).float().to(self.device)
            action = self.demonstrator(x)[0].cpu().data.numpy()
    ...
    def evaluate_action(self, state, goal, action_agent, action_demonstrator):
        state = self.state_normalizer.normalize(state)
        goal = self.goal_normalizer.normalize(goal)
        inputs = np.concatenate([state, goal])
        inputs = torch.Tensor(inputs).to(self.device)
        _action_agent = torch.Tensor(action_agent).to(self.device)
        _action_demonstrator = torch.Tensor(action_demonstrator).to(self.device)
        q_eval_agent = self.critic(inputs, _action_demonstrator)
        q_eval_demonstrator = self.critic(inputs, _action_demonstrator)
        q_eval_agent = q_eval_agent.cpu()[0].data.numpy()
        q_eval_demonstrator = q_eval_demonstrator.cpu()[0].data.numpy()
        if q_eval_demonstrator > q_eval_agent:  # choose action with higher Q
            action = action_demonstrator
        else:
            action = action_agent
        return action
    ...
    def init_target_networks(self):
        ...
        self.hard_update_networks(self.demonstrator, self.demonstrator_target)  # ---
    ...
    def train(self):
        inputs, rewards, next_inputs, actions = self.process_buffer_date()
        with torch.no_grad():
    ...
    def train_demonstrator(self):
        inputs, rewards, next_inputs, actions = self.process_buffer_date()
        with torch.no_grad():
            target_q = self.critic_target(next_inputs, self.demonstrator_target(next_inputs))  # --
            target_returns = rewards + self.gamma * target_q.detach()
            target_returns = torch.clamp(target_returns, -1 / (1 - self.gamma), 0)
        ...
        a = self.demonstrator(inputs)  # --
        actor_loss = -self.critic(inputs, a).mean()
        actor_loss += a.pow(2).mean()

        self.demonstrator_optim.zero_grad()  # --
        actor_loss.backward()
        self.sync_grads(self.demonstrator)  # --
        self.demonstrator_optim.step()  # --
    ...
    def process_buffer_date(self):
        ...
        return inputs, rewards, next_inputs, actions
    ...
    def save_weights(self):
        torch.save({"actor_state_dict": self.actor.state_dict(),
                    ...
                    "goal_normalizer_std": self.goal_normalizer.std}, "FetchReach")

    def load_weights(self):

        checkpoint = torch.load("FetchReach")
    ...
    def update_networks_demonstrator(self):
        self.soft_update_networks(self.demonstrator, self.demonstrator_target, self.tau)  # ---
        self.soft_update_networks(self.critic, self.critic_target, self.tau)
